...
function scheduleRecovery(ws,s,reason='queda inesperada'){
  ...
  if(nextAttempt>RECOVERY_DELAYS_MS.length){
    s.recovering=false;
    safeSend(ws,{type:'status',status:'disconnected',username:s.wantedUsername,mode:MODE,recoveryExhausted:true,attempt:s.recoveryAttempt,reason:'Auto Recovery esgotado. Reconecte manualmente.'});
    safeSend(ws,{type:'debug',event:'AUTO RECOVERY ESGOTADO',username:s.wantedUsername,attempt:s.recoveryAttempt,mode:MODE,at:Date.now()});
    return;
  }
  const delay=RECOVERY_DELAYS_MS[nextAttempt-1];
  s.recoveryAttempt=nextAttempt;s.recovering=true;
  ...
  s.recoveryTimer=setTimeout(async()=>{
    s.recoveryTimer=null;
    if(s.manualStop||s.connected||s.connecting||!s.wantedUsername||ws.readyState!==ws.OPEN||epoch!==s.generation){s.recovering=false;return}
    await startConnection(ws,s,s.wantedUsername,{recovery:true});
  },delay);
}

function makeLive(username){
...
async function startConnection(ws,s,username,{recovery=false}={}){
  ...
  await disposeLive(s,{clearUser:false,bumpGeneration:true});
  const generation=s.generation;
  s.username=username;s.connecting=true;s.connected=false;
  safeSend(ws,{type:'status',status:'checking',username,mode:MODE,recovery,attempt:recovery?s.recoveryAttempt:0});
  safeSend(ws,{type:'debug',event:recovery?'AUTO RECOVERY TENTANDO':'CONEXÃO MODERNA INICIADA',username,mode:MODE,attempt:recovery?s.recoveryAttempt:0,signerKey:Boolean(SIGN_API_KEY),at:Date.now()});
  const live=makeLive(username);s.live=live;attachEvents(ws,s,live,generation);
  try{
    const info=await live.connect();
    if(s.generation!==generation||s.live!==live){try{live.removeAllListeners?.();await live.disconnect?.()}catch{}return false}
    s.connecting=false;s.connected=true;s.hadConnected=true;s.recovering=false;s.recoveryAttempt=0;clearRecovery(s);
    safeSend(ws,{type:'status',status:'connected',username,roomId:info?.roomId||null,mode:MODE,recovered:recovery});
    safeSend(ws,{type:'debug',event:recovery?'LIVE RECUPERADA':'TIKTOK CONECTADA',username,roomId:info?.roomId||null,mode:MODE,at:Date.now()});
    return true;
  }catch(e){
    ...
    s.live=null;s.connecting=false;s.connected=false;s.recovering=false;
    if(recovery){
      safeSend(ws,{type:'debug',event:'AUTO RECOVERY FALHOU',username,attempt:s.recoveryAttempt,detail,mode:MODE,at:Date.now()});
      scheduleRecovery(ws,s,detail);
    }else{
      safeSend(ws,{type:'error',message:detail});
      safeSend(ws,{type:'status',status:'error',username,reason:detail,mode:MODE});
      safeSend(ws,{type:'debug',event:'CONEXÃO MODERNA FALHOU',username,detail,mode:MODE,at:Date.now()});
    }
    return false;
  }
}

async function connectOnce(ws,s,rawUsername){
  const username=String(rawUsername||'').trim().replace(/^@/,'');
  if(!username)return safeSend(ws,{type:'error',message:'Informe o @usuario da LIVE.'});
  if(s.connecting)return safeSend(ws,{type:'status',status:'checking',username:s.username||username,mode:MODE});
  if(s.connected&&s.live)return safeSend(ws,{type:'status',status:'connected',username:s.username,mode:MODE});
  clearRecovery(s);s.manualStop=false;s.recoveryAttempt=0;s.recovering=false;s.hadConnected=false;s.wantedUsername=username;
...
server.listen(PORT,'0.0.0.0',()=>console.log(`CAOS TIKTOK ${MODE} v${currentVersion()} online :${PORT} signerKey=${Boolean(SIGN_API_KEY)} recovery=bounded-2`));
