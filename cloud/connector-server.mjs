...
async function connectLive(ws,state,username){username=String(username||'').trim().replace(/^@/,'');if(!username)return safeSend(ws,{type:'error',message:'Informe o @usuario da LIVE.'});if(state.connecting)return safeSend(ws,{type:'error',message:'Conexão já em andamento.'});if(state.live&&state.username===username)return safeSend(ws,{type:'status',status:'connected',username});await closeLive(state);state.connecting=true;state.username=username;safeSend(ws,{type:'status',status:'connecting',username});const live=new TikTokLiveConnection(username,{enableExtendedGiftInfo:true});state.live=live;live.on(WebcastEvent.GIFT,data=>{const gift=normalizeGift(data);if(gift.giftType===1&&gift.repeatEnd===false)return;safeSend(ws,gift)});live.on(WebcastEvent.LIKE,data=>{const user=data.user?.uniqueId||data.uniqueId||data.nickname||'viewer';const count=Math.max(1,Number(data.likeCount||data.count||1)||1);safeSend(ws,{type:'gift',user,gift:'Curtida',count,giftId:'tiktok-like',repeatEnd:true,giftType:0})});live.on(WebcastEvent.CHAT,data=>safeSend(ws,{type:'chat',user:data.user?.uniqueId||data.uniqueId||'viewer',comment:data.comment||''}));live.on('disconnected',()=>safeSend(ws,{type:'status',status:'disconnected',username}));live.on('error',err=>safeSend(ws,{type:'error',message:err?.message||String(err)}));try{const info=await live.connect();state.connecting=false;safeSend(ws,{type:'status',status:'connected',username,roomId:info?.roomId||null})}catch(err){state.connecting=false;await closeLive(state);safeSend(ws,{type:'error',message:err?.message||String(err)})}}
