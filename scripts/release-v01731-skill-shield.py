...
def inject_before_function_end(src, signature, code):
...
g=inject_before_function_end(g,'function drawPlayer(){',"if(skillShieldP1())drawSkillShieldAt(0,0,'P1');")
ww('src/game.js',g)
...
# drawSoldier translates to q internally, so draw shield at local 0,0 before its closing brace
p=inject_before_function_end(p,'function drawSoldier(p,label,center=false){',"if(p.skillShield)drawSkillShield({x:0,y:0},label);")
ww('src/duo.js',p)

for f in ['index.html','painel.html','duo.html','map-lab.html']:
    s=rw(f).replace('v0.17.30','v0.17.31').replace('v=01730','v=01731')
    ww(f,s)
