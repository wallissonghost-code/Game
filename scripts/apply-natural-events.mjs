...
one(
  ...
  'event state/default meteor config'
);
...
one(
  ...
  "if(c==='eventdoublexp'){doubleXpAdmin=!!d.value;refreshEventFlags();toast(doubleXpAdmin?'✨ EVENTO 2× XP ADM ATIVADO':(doubleXpEvent?'✨ 2× XP ADM ENCERRADO · NATURAL CONTINUA':'✨ EVENTO 2× XP ENCERRADO · MOBS MARCADOS MANTÊM BÔNUS'))}",
  'admin double xp override'
);
...
one(
  ...
  'admin meteor override'
);

one(
  "hurtEnemy(e,dmg,kind);if(b.ice)",
  "hurtEnemy(e,dmg,kind,b.owner==='p2'?'p2':'p1');if(b.ice)",
  'bullet owner tracking'
);

one(
  "for(const e of hit){hurtEnemy(e,damage[lv],'arc');if(e.hp<=0&&!e.dead){e.dead=true;onKill(e,'p2')}}",
  "for(const e of hit){hurtEnemy(e,damage[lv],'arc','p2');if(e.hp<=0&&!e.dead){e.dead=true;onKill(e,'p2')}}",
  'duo arc owner tracking'
);

one(
  "drawPlayer();drawPhoenixShield();if(performance.now()>=phoenixShieldUntil)drawShield();drawFreeze();if(performance.now()<toastUntil)",
  "drawPlayer();drawPhoenixShield();if(performance.now()>=phoenixShieldUntil)drawShield();drawFreeze();drawEventHud();if(performance.now()<toastUntil)",
  'event hud hook'
);
...
one(
  ...
  'state event telemetry'
);

if(changes!==17) throw new Error(`unexpected patch count ${changes}`);
fs.writeFileSync(file,s);
console.log(`Natural events patch applied: ${changes} guarded replacements`);
