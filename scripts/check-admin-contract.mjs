...
// FPS visibility is a panel-local preference. Older panel code still sends an optional
// compatibility message, but the game does not need a command handler for it.
const localOnlyCommands=new Set(['fps']);
const unsupported=panelCommands.filter(c=>!gameCommands.includes(c)&&!localOnlyCommands.has(c));
if(!unsupported.length)ok('comandos ADM possuem handler no jogo',`${panelCommands.length-localOnlyCommands.size} remotos + ${localOnlyCommands.size} local`);else fail('painel envia comandos sem handler',unsupported.join(', '));

const criticalCommands=['pause','resume','heal','damage','clear','freeze','invincible','restart','spawn','boss','speed','xp','level','auto','autofire','gameplaymode','horde','skilltest','skilltestall','skillreset','eventdoublexp','eventmeteor','eventmeteorconfig','saveplayer'];
