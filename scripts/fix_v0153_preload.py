...
hmarker="const startButton=$('startBtn');if(startButton){startButton.disabled=false;startButton.textContent='ENTRAR NA ARENA';startButton.style.opacity='1';startButton.onclick=()=>reset()}"
hnew="const startButton=$('startBtn');if(startButton){syncStartButton();startButton.onclick=()=>reset()}"
if hmarker not in s: raise SystemExit('current start handler missing')
s=s.replace(hmarker,hnew,1)
