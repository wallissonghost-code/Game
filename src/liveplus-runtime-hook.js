...
function livePlusGhostTarget(g){
  ...
  const toPlayerX=player.x-g.x,toPlayerY=player.y-g.y,toPlayerLen=Math.hypot(toPlayerX,toPlayerY)||1;
  for(const e of enemies){
    if(e.dead)continue;
    const ex=e.x-g.x,ey=e.y-g.y,dist=Math.hypot(ex,ey);
    if(dist>500||dist<24)continue;
    const towardPlayer=(ex*toPlayerX+ey*toPlayerY)/(dist*toPlayerLen);
    if(towardPlayer>.12)continue;
    if(!livePlusGhostSegmentClear(g.x,g.y,e.x,e.y,player.x,player.y,82))continue;
    const boss=!!types[e.type]?.boss,score=dist-(boss?70:0);
...
function spawnLivePlusGhostAlly(d){
  ...
  const g={x:player.x+Math.cos(angle)*dist,y:player.y+Math.sin(angle)*dist,aim:angle+Math.PI,until:now+seconds*1000,born:now,nextShot:now+280+Math.random()*180,shotFlash:0,user};
...
function updateLivePlusGhostAllies(dt){
  ...
  for(const g of livePlusGhostAllies){
    ...
    if(!target){g.nextShot=now+220;continue}
    ...
    g.shotFlash=.12;
    g.nextShot=now+600+Math.random()*180;
  }
}
function drawLivePlusGhostAllies(){
  ...
  for(const g of livePlusGhostAllies){
    ...
    const dir=playerFacing(g.aim||0),pack=playerArmedReady?playerArmedFrames:playerBaseFrames,arr=pack[dir]?.length?pack[dir]:pack.down,img=arr?.[0]||soldierSprite;
    ctx.save();
    ctx.translate(p.x,p.y);
    ctx.globalAlpha=alpha;
    ctx.globalCompositeOperation='screen';
    const glow=ctx.createRadialGradient(0,0,8,0,0,42);
    glow.addColorStop(0,'rgba(125,211,252,.18)');glow.addColorStop(1,'rgba(125,211,252,0)');
    ctx.fillStyle=glow;ctx.beginPath();ctx.arc(0,0,42,0,Math.PI*2);ctx.fill();
    ctx.globalCompositeOperation='source-over';
    ctx.fillStyle='rgba(0,0,0,.35)';ctx.beginPath();ctx.ellipse(0,24,21,6,0,0,Math.PI*2);ctx.fill();
    if(img&&(img.naturalWidth||img.width)){
      const h=76,ratio=(img.naturalWidth&&img.naturalHeight)?img.naturalWidth/img.naturalHeight:.8,w=Math.min(75,h*ratio);
      ctx.imageSmoothingEnabled=true;ctx.drawImage(img,-w/2,34-h,w,h);
    }
    if(g.shotFlash>0){
      const m=muzzleLocal(dir);ctx.save();ctx.globalCompositeOperation='screen';ctx.fillStyle='#dff6ff';ctx.shadowColor='#38bdf8';ctx.shadowBlur=14;ctx.beginPath();ctx.arc(m.x*.78,m.y*.78,4,0,Math.PI*2);ctx.fill();ctx.restore();
    }
    ctx.globalAlpha=Math.min(1,alpha+.22);
    const txt='@'+g.user;ctx.font='800 9px -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';
    const tw=ctx.measureText(txt).width+14,ty=-54;ctx.fillStyle='rgba(5,8,18,.82)';ctx.strokeStyle='rgba(56,189,248,.78)';ctx.lineWidth=1;ctx.beginPath();ctx.roundRect(-tw/2,ty,tw,17,8);ctx.fill();ctx.stroke();ctx.fillStyle='#bae6fd';ctx.fillText(txt,0,ty+8.8);
    ctx.restore();
  }
}
`;
        ...
        out=out.replace('if(deathState)return;updateMeteorEvent(dt);','if(deathState)return;updateMeteorEvent(dt);updateLivePlusGhostAllies(dt);');
