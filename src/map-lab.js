...
function loadImage(def){return new Promise(resolve=>{const im=new Image();im.onload=()=>{state.images.set(def.id,im);resolve(true)};im.onerror=()=>resolve(false);im.src=`assets/Map/snow-frost/${def.file}?v=01728`})}
async function loadPack(){try{const res=await fetch('assets/Map/snow-frost/manifest.json?v=01728',{cache:'no-store'});if(!res.ok)throw Error('manifest '+res.status);const m=await res.json();state.manifest=m;state.byMask=Array.from({length:16},()=>[]);for(const d of m.chunks||[])if(Number.isInteger(d.mask)&&d.mask>=0&&d.mask<=15)state.byMask[d.mask].push(d);
