...
const COLORS=['#dcecf3','#9ed7e8','#3f8fb2','#c9d7df','#8ec8dc'];
const NAMES=['neve','gelo','lago gelado','trilha','ponte de gelo'];
...
const state={n:32,grid:[],seed:'GELO-001',zoom:1,panX:0,panY:0,drag:false,lastX:0,lastY:0,tiles:[],props:[],ready:false,stats:null};
...
function generate(){state.n=+$('#size').value;state.seed=$('#seed').value.trim()||'GELO-001';state.grid=new Array(state.n*state.n).fill(TYPE.forest);const water=+$('#water').value/100,clear=+$('#clearings').value/100,trails=+$('#trails').value;
...
function draw(){if(!state.grid.length)return;const r=wrap.getBoundingClientRect(),cell=32*state.zoom,total=state.n*cell,ox=r.width/2-total/2+state.panX,oy=r.height/2-total/2+state.panY;ctx.clearRect(0,0,r.width,r.height);ctx.fillStyle='#0c1820';ctx.fillRect(0,0,r.width,r.height);const showArt=$('#art').checked,showTypes=$('#types').checked,showGrid=$('#grid').checked,showMasks=$('#masks').checked,showErr=$('#errors').checked,showObj=$('#objects').checked;
...
function renderStats(){const s=state.stats;$('#cells').textContent=state.n*state.n;$('#roles').textContent=s.roles;$('#missing').textContent=s.special;$('#score').textContent=s.score+'%';const issues=$('#issues');issues.innerHTML='';const add=(c,t)=>issues.insertAdjacentHTML('beforeend',`<div class="issue ${c}">${t}</div>`);add('bad',`O pack de gelo atual tem <b>${state.tiles.length} tile base</b>, mas o mapa gerado pediu <b>${s.roles} combinações tipo+máscara</b>.`);add('warn',`<b>${s.special}</b> células precisam de borda, canto, reta, T ou peça isolada — hoje não há metadados que garantam o encaixe.`);add('good','A macrogeração já separa neve, gelo exposto, lagos congelados, trilhas e pontes de gelo. O próximo passo é produzir/classificar as peças correspondentes às máscaras mais usadas.');
