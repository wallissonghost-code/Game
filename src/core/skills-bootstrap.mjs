...
function fixClassicAimSync(source){
  ...
  const shootFn=`function shoot(){if(player.down||(choosing&&duoPlayer.connected))return false;let target=null,shotAim=player.aim;if(autoMode){target=focusedTarget();if(!target)return false;shotAim=Math.atan2(target.y-player.y,target.x-player.x)}else if(gameplayMode==='classic'){target=nearestVisible();if(!target)return false;shotAim=Math.atan2(target.y-player.y,target.x-player.x)}else if(gameplayMode==='sweep'){target=sweepTarget();if(!target)return false;shotAim=Math.atan2(target.y-player.y,target.x-player.x)}else if(gameplayMode==='hardcore'){shotAim=movementAimAngle}else{target=nearestVisible();if(!target)return false;shotAim=Math.atan2(target.y-player.y,target.x-player.x)}if(!Number.isFinite(shotAim))return false;let dir=playerFacing(shotAim),m=muzzleLocal(dir),spawnX=player.x+m.x,spawnY=player.y+m.y;if(target){shotAim=Math.atan2(target.y-spawnY,target.x-spawnX);dir=playerFacing(shotAim);m=muzzleLocal(dir);spawnX=player.x+m.x;spawnY=player.y+m.y;shotAim=Math.atan2(target.y-spawnY,target.x-spawnX)}if(!Number.isFinite(shotAim))return false;player.aim=shotAim;player.shotFlash=.1;const bs=berserkerState(),traits=window.CaosCombat.projectileTraits(skillLv,{pierce:pierceShotCounter,ice:iceShotCounter,explosive:explosiveShotCounter});pierceShotCounter=traits.counters.pierce;iceShotCounter=traits.counters.ice;explosiveShotCounter=traits.counters.explosive;const {pierceLeft,ice,explosive}=traits;ciShotsFired++;const shotId=ciShotsFired;if(pierceLeft)ciPierceShots++;if(ice)ciIceShots++;if(explosive)ciExplosiveShots++;const vx=Math.cos(shotAim)*610,vy=Math.sin(shotAim)*610;ciLastShot={shotId,spawnX,spawnY,playerX:player.x,playerY:player.y,aim:shotAim,vx,vy,targetX:target?.x??null,targetY:target?.y??null,targetType:target?.type??null,at:performance.now()};bullets.push({shotId,x:spawnX,y:spawnY,prevX:spawnX,prevY:spawnY,vx,vy,launchVx:vx,launchVy:vy,r:4,dead:false,ammo:1,born:performance.now(),pierceLeft,hits:[],iceHits:0,damage:player.damage*bs.damageMul,ice,explosive,owner:'p1'});if(player.flashDamage&&++flashCounter%5===0)flash();return true}`;
...
async function loadPatchedClassic(src){const response=await fetch(src,{cache:'no-store'});if(!response.ok)throw new Error(`Failed to fetch runtime ${response.status}`);const original=await response.text();const patched=fixClassicAimSync(patchNaturalEvents(original));const blobUrl=URL.createObjectURL(new Blob([patched],{type:'text/javascript'}));try{await loadClassic(blobUrl)}finally{URL.revokeObjectURL(blobUrl)}}
