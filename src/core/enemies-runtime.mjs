...
export function buildEnemySpawn({type,typeConfig,near=false,forcedTier=null,level=1,player,viewportWidth=0,viewportHeight=0,doubleXpEvent=false,enemyCount=0,maxEnemies=MAX_ENEMIES,mobDomain,random=Math.random}={}){
  if(enemyCount>=maxEnemies||!typeConfig||!player||!mobDomain)return null;
  ...
  const variant=mobDomain.variantFor({boss:!!c.boss,tier,evolution}),hpMult=variant.hp,dmgMult=variant.dmg,xpMult=variant.xp,hitboxMult=c.boss?1:variant.hitbox;
...
export function assertEnemyState(){
  ...
  if(!built||built.enemy.type!=='wraith'||built.enemy.hp!==30||built.enemy.x!==180)throw new Error('CAOS enemy spawn builder invalid');
  return true;
}
