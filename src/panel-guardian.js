...
function installMobileTapSafety(){
  // iOS Safari can interpret rapid admin taps as double-tap zoom. We block only
  // double-click zoom on actionable controls, preserving normal pinch zoom/accessibility.
...
function proxyCommand(command,patch={}){
  const selector=`[data-cmd="${command}"]`;
  const source=document.querySelector(selector);
  if(!source)return false;
  const old={};
  for(const [k,v] of Object.entries(patch)){
    old[k]=source.dataset[k];
    if(v===undefined||v===null)delete source.dataset[k];else source.dataset[k]=String(v);
  }
  source.click();
  for(const [k,v] of Object.entries(old)){
    if(v===undefined)delete source.dataset[k];else source.dataset[k]=v;
  }
  return true;
}
...
function installLiveDirector(){
  ...
  const anchor=document.querySelector('.automationsCard');
  if(!anchor)return;
  const card=document.createElement('section');
  card.id='caosLiveDirector';card.className='caosQuickCard';card.dataset.adminArea='live';
  ...
  grid.append(
    ...
    makeButton('👾 +10 MOBS','caos rápido','live',()=>{
      const amount=$('mobAmount');const old=amount?.value;if(amount)amount.value='10';$('spawn')?.click();if(amount)amount.value=old;
    }),
    makeButton('👾 +50 MOBS','pico de interação','live',()=>{
      const amount=$('mobAmount');const old=amount?.value;if(amount)amount.value='50';$('spawn')?.click();if(amount)amount.value=old;
    }),
...
function classifyAreas(){
  ...
  if(dashboard){
    dashboard.querySelector('.connectionCard')?.setAttribute('data-admin-area','partida');
    dashboard.querySelector('.cloudCard')?.setAttribute('data-admin-area','live');
  }
  ...
  if(controls){
    const cards=[...controls.children].filter(x=>x.classList?.contains('card'));
...
function installNavigation(){
  ...
  const apply=area=>{
    document.querySelectorAll('[data-admin-area]').forEach(el=>el.classList.toggle('adminHidden',el.dataset.adminArea!==area));
    nav.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.area===area));
    try{localStorage.setItem('caos-admin-area',area)}catch{}
  ...
  apply(initial);
  // Live monitor is inserted after this module; classify it when it appears.
