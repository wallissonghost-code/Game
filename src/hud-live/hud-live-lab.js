(()=>{
  ...
  function renderHud(){
    dock.innerHTML=state.length?state.slice(0,4).map(g=>`<div class="hudLiveItem"><div class="hudLiveGiftIcon">${icon(g)}</div><div class="hudLiveText"><div class="hudLiveGiftName">${esc(g.name||'Presente')}</div><div class="hudLiveEffect"><b>${esc(g.effect||'EFEITO')}</b></div></div></div>`).join(''):'<div class="hudLiveEmpty">NENHUM COMANDO VISÍVEL</div>';
  }
  ...
  document.getElementById('hudLiveReset').addEventListener('click',()=>{state=Store.reset();renderEditor();renderHud()});
